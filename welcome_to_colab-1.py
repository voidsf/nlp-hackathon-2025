...
st.markdown("An interactive dashboard to track and analyze developing news stories in real-time.")
...
def get_ai_summary(articles_text):
    ...
    """
    prompt = f"""
    Based on the following collection of recent news article summaries, please act as a business intelligence analyst.
    Provide a concise executive summary (3-4 sentences) that highlights the key events, the main actors (people and organizations) involved, and the overall sentiment or direction of the developments.
    ...
    apiKey = "AIzaSyD5D0vquzptzIgq769tXU5En4TwyRr0giI"
...
if df.empty:
    ...
    # --- NEW: LOGIC TO AUTO-CLEAR SUMMARY ON FILTER CHANGE ---
    current_filters = {
        "range": selected_range_label,
        "people": tuple(sorted(selected_people)),
        "orgs": tuple(sorted(selected_orgs))
    }
    ...
    if st.session_state.last_filters != current_filters:
        if "summary" in st.session_state:
            del st.session_state.summary # Clear old summary
        st.session_state.last_filters = current_filters # Update to new filters
            ...
            try:
                ...
                with st.expander("Deep Dive: Selected Article", expanded=True):
                    ...
                    if 'highlights' in article_details and article_details['highlights']:
                        st.markdown("**Key Highlights from API:**")
